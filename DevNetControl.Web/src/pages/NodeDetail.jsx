...
import { Server, Loader2, AlertCircle, CheckCircle, Terminal, Activity, Trash2, Wifi } from 'lucide-react'
...
export default function NodeDetail() {
  ...
  async function handleFetchMetrics() {
    ...
    try {
      ...
      setMessage({ type: 'error', text: err.response?.data?.message || 'Error obteniendo métricas' })
    } finally {
      setActionLoading(false)
    }
  }
  ...
  if (!node) {
    return (
      <div className="text-center py-12">
        ...
        <p className="text-gray-500">Nodo no encontrado</p>
      </div>
    )
  }
    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-2">
        <button
          ...
          className="flex items-center justify-center gap-2 bg-white border border-gray-200 py-3 rounded-xl font-medium hover:bg-gray-50 transition-colors disabled:opacity-50"
        >
        ...
        <form onSubmit={handleExecuteCommand} className="p-4 space-y-3">
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-3 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-transparent outline-none font-mono text-sm"
              placeholder="uptime"
              required
            />
            <button
